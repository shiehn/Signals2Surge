...
MIDI messages use the format: ([status, data1, data2], timestamp_seconds)
"""
...
def _status(msg):
    """Extract status byte from a MIDI message."""
    return msg[0][0]


def _note(msg):
    """Extract note number from a MIDI message."""
    return msg[0][1]


def _velocity(msg):
    """Extract velocity from a MIDI message."""
    return msg[0][2]


def _time(msg):
    """Extract timestamp from a MIDI message."""
    return msg[1]
...
class TestCreateProbe:
    ...
    def test_default_note_on(self):
        messages = create_probe()
        msg = messages[0]
        assert _time(msg) == 0.0
        assert _status(msg) == NOTE_ON
        assert _note(msg) == 60  # C4
        assert _velocity(msg) == 100

    def test_default_note_off(self):
        messages = create_probe()
        msg = messages[1]
        assert _time(msg) == 3.0  # sustain_seconds
        assert _status(msg) == NOTE_OFF
        assert _note(msg) == 60

    def test_custom_config(self):
        config = MidiProbeConfig(note=72, velocity=80, sustain_seconds=2.0, release_seconds=0.5)
        messages = create_probe(config)
        assert messages[0] == ([NOTE_ON, 72, 80], 0.0)
        assert messages[1] == ([NOTE_OFF, 72, 0], 2.0)
...
class TestSustainedProbe:
    ...
    def test_sustained_notes_sequential(self):
        ...
        first_off_time = _time(msgs[1])
        second_on_time = _time(msgs[2])
...
class TestVelocityProbe:
    ...
    def test_velocity_values_correct(self):
        ...
        note_ons = [m for m in msgs if _status(m) == NOTE_ON]
        assert [_velocity(m) for m in note_ons] == [30, 80, 127]
...
class TestStaccatoProbe:
    ...
    def test_staccato_short_notes(self):
        ...
        for i in range(0, len(msgs), 2):
            on_time = _time(msgs[i])
            off_time = _time(msgs[i + 1])
...
class TestChordProbe:
    def test_chord_three_notes_simultaneous_on(self):
        ...
        # All note-ons at t=0
        assert all(_time(m) == 0.0 for m in note_ons)

    def test_chord_three_notes_simultaneous_off(self):
        msgs = create_chord_probe([60, 64, 67], velocity=90, sustain_s=1.5, release_s=0.8)
        note_offs = [m for m in msgs if _status(m) == NOTE_OFF]
        assert len(note_offs) == 3
        # All note-offs at same time
        assert all(_time(m) == 1.5 for m in note_offs)
...
class TestMelodyProbe:
    def test_melody_eight_notes_ascending(self):
        ...
        assert [_note(m) for m in note_ons] == notes
...
class TestLegatoProbe:
    def test_legato_notes_overlap(self):
        ...
        note_ons = sorted([m for m in msgs if _status(m) == NOTE_ON], key=_time)
        note_offs = sorted([m for m in msgs if _status(m) == NOTE_OFF], key=_time)
        # Note N+1 starts before note N ends
        for i in range(len(note_ons) - 1):
            next_on_time = _time(note_ons[i + 1])
            current_off_time = _time(note_offs[i])
...
class TestIntervalJumpProbe:
    def test_interval_jump_wide_pitch_range(self):
        ...
        note_ons = [m for m in msgs if _status(m) == NOTE_ON]
        pitches = [_note(m) for m in note_ons]
...
class TestComposeMultiProbe:
    def test_compose_single_matches_legacy(self):
        ...
        for composed, orig in zip(result.midi_messages, legacy):
            assert composed[0] == orig[0]  # midi bytes
            assert composed[1] == orig[1]  # timestamp
    ...
    def test_all_timestamps_monotonic(self):
        config = MultiProbeConfig.full()
        result = compose_multi_probe(config, sample_rate=44100)
        timestamps = [_time(m) for m in result.midi_messages]
